...
namespace APBox.Controllers.Ajax
{
    public class AjaxOperacionesController : Controller
    {
        ...
        public int ValidaUuidCfdis(string UUID,int RECEPTOR , int EMISOR)
        {
            int result = 0;
            var cfdiRelacionado = _db.FacturasEmitidas.Where(f => f.Uuid == UUID && f.ReceptorId == RECEPTOR && f.EmisorId == EMISOR).FirstOrDefault();
