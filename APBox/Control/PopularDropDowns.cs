...
namespace APBox.Control
{
    public class PopularDropDowns
    {
        ...
        {
            //var t = _db.Cat_Impuestos.Where(a => a.TipoImpuesto == "Traslado" && a.SucursalId == sucursalId);
            //return new SelectList(_db.Cat_Impuestos.Where(a => a.TipoImpuesto == "Traslado" && a.SucursalId == sucursalId), "Id", "TasaOCuota");
        ...
        {
            //var r = _db.Cat_Impuestos.Where(a => a.TipoImpuesto == "Retencion" && a.SucursalId == sucursalId);
            //return new SelectList(_db.Cat_Impuestos.Where(a => a.TipoImpuesto == "Retencion" && a.SucursalId == sucursalId), "Id", "TasaOCuota");
        ...
        public SelectList PopulaCfdiRelacionados(bool soloPpd, int? clienteId, int? seleccion)
        {

            var facturasCliente = _db.FacturasEmitidas.Where(a => a.EmisorId == _entidadId && a.ReceptorId == clienteId && a.Total > 0).ToList();
            var ListCfdiR = new  List<CfdiRelacionadoDto>();
            foreach(var f in facturasCliente)
            {
                CfdiRelacionadoDto cfdiR = new CfdiRelacionadoDto 
                {
                    Id = f.Id,
                    Desplegado = f.Desplegado
                };
                ListCfdiR.Add(cfdiR);
            }
            //var facturasFinales = facturasCliente.ToDictionary(f => f.Id , f => f.Desplegado);
            return new SelectList(ListCfdiR, "Id", "Desplegado", (int)seleccion);
            //return new SelectList(facturasFinales, "key", "value",seleccion);
        }
